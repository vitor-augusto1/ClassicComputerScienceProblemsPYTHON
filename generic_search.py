...
import concurrent
from typing import (Deque, Dict, Optional, TypeVar, Iterable, Sequence, Generic, List,
...
def dfs(initial: T, goal_test: Callable[[T], bool],
    ...
    # Frontiers are the places that haven't been visited
    frontier: Stack[Node[T]] = Stack()
    frontier.push(Node(initial, None))
    # Explored are the places that have been visited
...
class PriorityQueue(Generic[T]):
    ...
    def push(self, item: T) -> None:
        heappush(self._container, item)  # insert according to priority

    def pop(self) -> T:
        return heappop(self._container)  # remove according to priority

    def __repr__(self) -> str:
        return repr(self._container)


def astar(initial: T, goal_test: Callable[[T], bool], sucessors: Callable[[T],
